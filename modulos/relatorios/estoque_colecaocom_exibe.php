...
<?
	...
	<tr>
		...
		if ($db->num_rows ( $query )) {
			?>
			<a href="javascript:;"
			onclick="javascript:document.getElementById('colecao_<?=$rowcolecao ['idcolecao'];?>').style.display = ((document.getElementById('colecao_<?=$rowcolecao ['idcolecao'];?>').style.display=='none')?'block':'none');"><b><?=ucwords ( strtolower ( $rowcolecao ['txtnome'] ) );?> - <?=$rowcolecao ['txtperiodo'];?></b></a>
			<?
		} else {
			?>
			<b><?=ucwords ( strtolower ( $rowcolecao ['txtnome'] ) );?> - <?=$rowcolecao ['txtperiodo'];?></b>
			<?
		}
		?>
		</td>
...
<?
	...
	<tr>
		<td>
		<div id="colecao_<?=$rowcolecao ['idcolecao'];?>"
			style="display: none;">
		<table width="100%">
			<tr>
...
<?
	...
	echo exibe_errohtml ( 'Nenhuma coleção com produto em estoque' );
