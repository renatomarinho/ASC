...
if ($db->num_rows ( $queryfornecedor )) {
	...
	<tr>
		...
		if ($db->num_rows ( $query )) {
			?>
			<a href="javascript:;"
			onclick="javascript:document.getElementById('fornecedor_<?=$rowfornecedor ['idfornecedor'];?>').style.display = ((document.getElementById('fornecedor_<?=$rowfornecedor ['idfornecedor'];?>').style.display=='none')?'block':'none');"><b><?=ucwords ( strtolower ( $rowfornecedor ['nome'] ) );?></b></a>
			<?
		} else {
			?>
...
<?
	...
	<tr>
		<td>
		<div id="fornecedor_<?=$rowfornecedor ['idfornecedor'];?>"
			style="display: none;">
		<table width="100%">
			<tr>
...
<?
	...
	echo exibe_errohtml ( 'Nenhum fornecedor sem produto em estoque' );
