...
$sql = "SELECT DISTINCT(id_produto) FROM mv_vendas_movimento WHERE estornado=0 AND data_venda BETWEEN '" . $data1 . "' AND '" . $data2 . "' ORDER BY data_venda ASC";
...
<?
	...
	<tr onmouseover="rowOver(this);document.getElementById('produto_<?=$rows ['idproduto'];?>').style.backgroundColor='<?=$_CONF ['bg_row_over'];?>';" onmouseout="rowOut(this);document.getElementById('produto_<?=$rows ['idproduto'];?>').style.backgroundColor='<?=$_CONF ['bg_row_out'];?>';" style="cursor: pointer; cursor: hand;" onclick="javascript:document.getElementById('produto_<?=$rows ['idproduto'];?>').style.display = ((document.getElementById('produto_<?=$rows ['idproduto'];?>').style.display=='none')?'block':'none');">
		<td width="120" align="center">
		<div class="hidden_export"><a href="javascript:;" onclick="javascript:carrega_dadosproduto('?refer=listagem&id=<?=$rows ['idproduto'];?>');">[ hist�rico ]</a></div>
		</td>
	...
	<tr>
		<td colspan="5">
		<div id="produto_<?=$rows ['idproduto'];?>" class="show_export">
		<?
