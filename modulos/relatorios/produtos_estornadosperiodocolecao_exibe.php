...
while ( $rowcolecao = $db->fetch_assoc ( $querycolecao ) ) {
	...
	$sql = "SELECT DISTINCT mv.id_produto FROM mv_vendas_movimento AS mv INNER JOIN produto AS p ON mv.id_produto=p.idproduto WHERE p.colecao_idcolecao=" . $rowcolecao ['idcolecao'] . " AND mv.data_venda BETWEEN '" . $data1 . "' AND '" . $data2 . "'";
...
<table width="100%">
	<tr>
		...
		<table width="100%">
			<tr>
				<td><b>Coleção :</b> <a href="javascript:;"
					onclick="javascript:document.getElementById('produtocol_<?=$rowcolecao ['idcolecao'];?>').style.display = ((document.getElementById('produtocol_<?=$rowcolecao ['idcolecao'];?>').style.display=='none')?'block':'none');"><b><?=ucwords ( strtolower ( $rowcolecao ['txtnome'] ) );?></b></a></td>
	...
	<tr>
		<td>
		<div id="produtocol_<?=$rowcolecao ['idcolecao'];?>"
			style="display: none;">
