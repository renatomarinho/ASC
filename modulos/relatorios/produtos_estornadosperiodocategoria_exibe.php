...
while ( $rowcategoria = $db->fetch_assoc ( $querycategoria ) ) {
	...
	$sql = "SELECT DISTINCT mv.id_produto FROM mv_vendas_movimento AS mv INNER JOIN produto AS p ON mv.id_produto=p.idproduto WHERE mv.estornado=1 AND p.produtotipo_idprodutotipo=" . $rowcategoria ['idprodutotipo'] . " AND mv.data_venda BETWEEN '" . $data1 . "' AND '" . $data2 . "'";
