...
while ( $rowfornecedor = $db->fetch_assoc ( $queryfornecedor ) ) {
	...
	$sql = "SELECT DISTINCT mv.id_produto FROM mv_vendas_movimento AS mv INNER JOIN produto AS p ON mv.id_produto=p.idproduto WHERE mv.estornado=1 AND p.fornecedor_idfornecedor=" . $rowfornecedor ['idfornecedor'] . " AND mv.data_venda BETWEEN '" . $data1 . "' AND '" . $data2 . "'";
...
<table width="100%">
	...
	<tr>
		<td>
		<div id="produtofor_<?=$rowfornecedor ['idfornecedor'];?>"
			style="display: none;">
