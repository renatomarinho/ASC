...
$sql = "SELECT v.data_venda, SUM(v.vr_totalvenda) AS vr_totalvenda, v.vr_opcionalvenda, SUM(mv.vr_custo*mv.quant) AS vr_custo  FROM mv_vendas AS v INNER JOIN mv_vendas_movimento AS mv ON v.controle=mv.controle WHERE v.data_venda BETWEEN '".$data1."' AND '".$data2."' GROUP BY v.data_venda";
$query = $db->query($sql);
