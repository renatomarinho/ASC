...
<table width="100%">
	<tr>
		...
		<td align="right"><input type="button" value="Ir para listagem"
			...
			onclick="javascript:carrega_listagemcolecoes(path+'modulos/colecao/colecao_lista.php','conteudo_direito', 'comparacao');"></td>
	</tr>
</table>
</div>
...
<table width="100%">
	...
	<tr>
		...
		<div style="width: 340px;">
					...
					$remonta_url = '';
					for($i = 0; $i < $total; $i ++) {
						$remonta_url .= '&idcol' . $i . '=' . $_GET ['idcol' . $i];
					}
					require $_CONF ['PATH'] . 'modulos/charts/library/open_flash_chart_object.php';
					open_flash_chart_object ( 340, 280, $_CONF ['PATH_VIRTUAL'] . 'modulos/charts/chart-comparacolecao.php?total=' . $total . '&param=' . $param . $remonta_url, false );
					?>
	...
	<tr>
		...
		<table width="100%">
				...
				foreach ( $vetor_url as $key => $value ) {
					...
					?>
					<tr>
				<td height="25"><?=qtdCaracteres ( ucwords ( strtolower ( $rowcolecao ['txtnome'] ) ), 30 )?></td>
				<td align="right">
						<?
					$resultado = resultadorelatorio ( $vetor_url [$key], $param );
					if ($param == 1) {
						if ($resultado > 0)
